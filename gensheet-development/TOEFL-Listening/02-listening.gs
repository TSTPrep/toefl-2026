// TOEFL 2026 - Listening Section - 4-in-1 Passage Generator
// This script generates passages and questions for four different TOEFL Listening exercise types.
...
function generateBatch_Choose() { generatePassageBatch('Choose'); }
function startBatchProcess_Choose() { startBatchRun('Choose'); }
function startEditSheetConversions_Choose() { startEditSheetConversions('Choose'); }
function deleteEditingSheets_Choose() { deleteEditingSheets('Choose'); }

function generateBatch_Conversation() { generatePassageBatch('Conversation'); }
function startBatchProcess_Conversation() { startBatchRun('Conversation'); }
function startEditSheetConversions_Conversation() { startEditSheetConversions('Conversation'); }
function deleteEditingSheets_Conversation() { deleteEditingSheets('Conversation'); }

function generateBatch_Announcement() { generatePassageBatch('Announcement'); }
function startBatchProcess_Announcement() { startBatchRun('Announcement'); }
function startEditSheetConversions_Announcement() { startEditSheetConversions('Announcement'); }
function deleteEditingSheets_Announcement() { deleteEditingSheets('Announcement'); }

function generateBatch_AcademicTalk() { generatePassageBatch('AcademicTalk'); }
function startBatchProcess_AcademicTalk() { startBatchRun('AcademicTalk'); }
function startEditSheetConversions_AcademicTalk() { startEditSheetConversions('AcademicTalk'); }
function deleteEditingSheets_AcademicTalk() { deleteEditingSheets('AcademicTalk'); }
  ...
  try {
    ...
    try {
      ...
      const outputData = contentArray.map((content, index) => {
          ...
          // This loop dynamically finds and processes question data from the AI response.
          // It supports both simple formats { "question": "..." } and complex ones
          // with nested objects like { "question1": {...}, "question2": {...} }.
          for (let i = 1; i <= 10; i++) { // Check for up to 10 question sets per item
              const questionKey = `question${i}`;

              if (content[questionKey]) {
                  // Handles nested question sets (e.g., "question1": { ... })
                  const questionSet = content[questionKey] || {};
                  rowData.push(
                      questionSet.question || `[Missing Q${i}]`,
                      questionSet.answer || `[Missing A${i}]`,
                      ...(questionSet.distractors || ["[M]", "[M]", "[M]"])
                  );
              } else if (i === 1 && content.question) {
                  // Handles the simple, flat format (e.g., "question": "...")
                  // This ensures backward compatibility with the 'Choose' exercise.
                  rowData.push(
                      content.question || "[Missing Q1]",
                      content.answer || "[Missing A1]",
                      ...(content.distractors || ["[M]", "[M]", "[M]"])
                  );
                  // Since this format has only one question, we stop looking for more.
                  break;
              }
          }
          return rowData;
...
function startEditSheetConversions(exercise) {
  ...
  for (let i = startRow; i <= endRow; i++) {
    ...
    // --- Populate Questions, Answers, and Distractors ---
    
    // Starting index in sourceData for the first question (Column C is index 2)
    let questionDataIndex = 2; 

    // Starting row numbers in the template sheet for the first question block
    let questionRow = 9;
    let answerRow = 12;
    let distractorStartRow = 15;

    // Loop through the questions as long as there's data for them in the source row
    while (questionDataIndex < sourceData.length && sourceData[questionDataIndex]) {
        // Set Question (from C, H, M, R, etc.)
        newSheet.getRange('B' + questionRow).setValue(sourceData[questionDataIndex]);

        // Set Answer (from D, I, N, S, etc.)
        newSheet.getRange('B' + answerRow).setValue(sourceData[questionDataIndex + 1]);

        // Set Distractors (from EFG, JKL, OPQ, TUV, etc.)
        newSheet.getRange('B' + distractorStartRow).setValue(sourceData[questionDataIndex + 2]);
        newSheet.getRange('B' + (distractorStartRow + 1)).setValue(sourceData[questionDataIndex + 3]);
        newSheet.getRange('B' + (distractorStartRow + 2)).setValue(sourceData[questionDataIndex + 4]);

        // Update indices and row numbers for the next question block
        questionDataIndex += 5;      // Move 5 columns to the right for the next question
        questionRow += 11;           // Move down 11 rows in the template for the next question
        answerRow += 11;
        distractorStartRow += 11;
    }
  }
